...
function SWEP:DoBobFrame()
	...
	local owvel, meetswalkgate, meetssprintgate, walkfactorv, runfactorv, sprinrevctorv
	...
	if owvel>1 and owvel<=self.Owner:GetWalkSpeed()*0.1 and owvel>self.tprevvel then
		...
		if self.Owner:IsOnGround() then
			self.bobtimevar = self.bobtimevar + ftv * math.max(1, owvel / (runfactorv + (sprinrevctorv-runfactorv)*(meetssprintgate and 1 or 0)  - (runfactorv-walkfactorv)*(meetswalkgate and 1 or 0) ) )
		else
