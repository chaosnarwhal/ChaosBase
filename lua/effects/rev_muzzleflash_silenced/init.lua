...
function EFFECT:Init( data )
	...
	local emitter = ParticleEmitter( self.vOffset )
		for i=0, 2 do
			local particle = emitter:Add( "particles/flamelet"..math.random(1,5), self.vOffset + (dir * 2 * i))
			if (particle) then
				...
				particle:SetStartSize( 4 - 1 * i )
				particle:SetEndSize( 0 )
				particle:SetRoll( math.Rand(0, 360) )
				particle:SetRollDelta( math.Rand(-15, 15) )
				particle:SetColor( 255 , 230 , 115 )
		...
		for i=0, 7 do
			...
			local particle = emitter:Add( "particles/smokey", self.vOffset + dir * math.Rand(0, 4 ))
			if (particle) then
		...
		if GetConVarNumber("cl_rev_fx_gasblur",1)==1 then
			for i=0, 3 do
