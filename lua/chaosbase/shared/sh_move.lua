...
local function FinishMove(ply, cmovedata)
	...
	if not wep:GetSafety() then 
		if band(changed, IN_ATTACK2) == IN_ATTACK2 then
			...
			elseif wep:GetIsAiming() and
				((chaosbase_ironsights_toggle or chaosbase_ironsights_responsive) and band(pressed, IN_ATTACK2) == IN_ATTACK2 or 
					not chaosbase_ironsights_toggle and not chaosbase_ironsights_responsive and band(depressed, IN_ATTACK2) == IN_ATTACK2)
			then
...
hook.Add("SetupMove", "Movement", function(ply, mv, cmd )
    ...
    if ply:WaterLevel() > 0 and ply:WaterLevel() <= 2 and !ply:OnGround() then
...
hook.Add( "SetupMove", "JumpHeight", function( ply, mv, cmd )
    ...
    if !jump_enabled then return end
    if ply:KeyDown(IN_JUMP) and !ply:OnGround() and mv:GetVelocity()[3] > 0 then
